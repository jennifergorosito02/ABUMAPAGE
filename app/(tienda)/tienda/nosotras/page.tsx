...
export default function NosotrasPage() {
  ...
  useEffect(() => {
    supabase.from('configuracion').select('texto_nosotras').eq('id', 1).single()
      .then(({ data }) => { setTexto(data?.texto_nosotras ?? null); setLoading(false) })
  }, [])
  ...
  return (
    <div style={{ minHeight: '80vh', maxWidth: '760px', margin: '0 auto', padding: 'clamp(40px, 8vw, 100px) 24px' }}>
      ...
      <div style={{ marginTop: '40px', marginBottom: '48px' }}>
        <div style={{ fontSize: '10px', letterSpacing: '0.3em', color: 'rgba(200,169,110,0.5)', textTransform: 'uppercase', marginBottom: '12px' }}>Quiénes somos</div>
