        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          {fotos.length > 1 && !modoOrden && (
            <div className="miniaturas-desktop" style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              {fotos.map((url, i) => (
                <button key={i} onClick={() => setFotoIdx(i)} style={{ width: '64px', height: '64px', borderRadius: '8px', overflow: 'hidden', padding: 0, cursor: 'pointer', border: `2px solid ${i === fotoIdx ? 'var(--gold)' : 'var(--border)'}`, flexShrink: 0 }}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
